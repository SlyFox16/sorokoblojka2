...
{	
	...
	public function ClearSessions()
	{
		...
		$t = "time_last < '%d'";
		$where = sprintf($t, $min);
		$this->msql->Delete(DB_PREF.'sessions', $where);
	}
	...
	public function Login($login, $password, $remember)
	{
		...
		switch($remember)
		{
			case "true": $expire = time() + 3600 * 24 * 100;
			setcookie('login', $login, $expire, '/');
			setcookie('password', md5($user['salt'].$password), $expire, '/'); break;
		}
		/*if ($remember)
		{
			$expire = time() + 3600 * 24 * 100;
			setcookie('login', $login, $expire, '/');
			setcookie('password', md5($user['salt'].$password), $expire, '/');
		}	*/	
	...
	{	
		...
		$t = "SELECT * FROM ".DB_PREF."users WHERE id_user = '%d'";
...
/*        public function GetAll()
	{
		$query = "SELECT * FROM ".$this->tbl_prefix."_users";
	...
	{	
		$t = "SELECT * FROM ".DB_PREF."users WHERE login = '%s'";
	...
	public function Can($priv, $id_user = null)
	{
		...
		if ($id_user == null)
			...
			return false;
                ...
                $query = "SELECT count(*) FROM " . DB_TBL_PREF . "_privs2roles
			    		 JOIN " . DB_TBL_PREF . "users 
			  	   			   ON " . DB_TBL_PREF . "users.id_role = " . DB_TBL_PREF . "privs2roles.id_role 
			  	   			   AND " . DB_TBL_PREF . "users.id_user = '%d'
			 			 JOIN " . DB_TBL_PREF . "privs 
			  	  			   ON " . DB_TBL_PREF . "privs.id_priv = " . DB_TBL_PREF . "privs2roles.id_priv 
			  	   			   AND " . DB_TBL_PREF . "privs.name = '%s'";
	...
	{	
		...
		$t = "SELECT id_user FROM ".DB_PREF."sessions WHERE sid = '%s'";
	...
	private function GetSid()
	{
		...
		if ($sid != null)
		{
			...
			$affected_rows = $this->msql->Update(DB_PREF.'sessions', $session, $where);
			
			if ($affected_rows == 0)
			{
				$t = "SELECT count(*) as con FROM ".DB_PREF."sessions WHERE sid = '%s'";
	...
	private function OpenSession($id_user)
	{
		...
		$this->msql->Insert(DB_PREF.'sessions', $session);
	...
	//
	private function GenerateStr($length = 16) 
	{
